...
![TestOps Export UI](doc/testops_export_view.png)
...
## Конфигурация проектов и групп для экспорта

Для управления списком проектов и групп используется отдельный файл `projects.json` (или другой путь, указанный в переменной окружения `PROJECTS_CONFIG`).

### Пример файла projects.json
  ...
  "projects": [
    {
      ...
      "tree_id": 937,
      "groups": [
        { "group_id": 26961091, "group_name": "API" },
        { "group_id": 24545654, "group_name": "UI" },
        { "group_id": 30308896, "group_name": "UI-AT" }
      ]
    },
    {
      "project_id": 15,
      "tree_id": 868,
      "groups": [
        { "group_id": 21360405, "group_name": "Seller-Analitics" }
      ]
    }
  ]
}
...
- `project_id` — ID проекта в TestOps
- `tree_id` — ID дерева тест-кейсов в проекте
- `groups` — список групп для экспорта (ID и имя)

### Как указать путь к файлу проектов

В `.env` (или переменных окружения) добавьте:
```
PROJECTS_CONFIG=projects.json
```

Если переменная не указана, будет использоваться файл `projects.json` в корне проекта по умолчанию.

### Как добавить/изменить проекты и группы

1. Откройте `projects.json` в любом редакторе.
2. Добавьте или измените нужные проекты, tree_id и группы.
3. Перезапустите приложение — новые настройки будут подхвачены автоматически.

---

### Как получить project_id, tree_id и group_id для экспорта

1. Откройте нужный проект и папку/группу тест-кейсов в TestOps.
2. Нажмите на "три точки" рядом с нужной группой и выберите "Экспорт → CSV".
3. Откройте DevTools (F12) → вкладка Network → найдите запрос `csv`.
4. В Payload этого запроса будут нужные параметры:
   - `projectId` — ID проекта
   - `treeId` — ID дерева тест-кейсов
   - `groupsInclude` — список groupId (ID группы)

Пример:

![Пример получения projectId, treeId и groupId в TestOps](./doc/testops_group_info_example.png)

На скриншоте видно, как получить все нужные параметры для настройки экспорта.
